...
in
{
  ...
  systemd.user.services."podman-unbound" = {
    serviceConfig = {
  ...
  systemd.user.services."podman-unbound-redis" = {
    serviceConfig = {
      Restart = lib.mkOverride 500 "always";
    };
    after = [
  ...
  # Networks
  systemd.user.services."podman-network-unbound_default" = {
    path = [ pkgs.podman ];
    serviceConfig = {
      Type = "oneshot";
  ...
  systemd.user.targets."podman-compose-unbound-root" = {
    unitConfig = {
